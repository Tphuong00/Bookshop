...
cloudinary.config({
  cloud_name: "ilike",
  api_key: "614535246148838",
  api_secret: "emJ9mdCYiPYgIz51URz6xSVVjvA",
});
...
exports.updateBook = async (req, res) => {
  ...
  let { name, id, id_category, price, release_date, describe, quantity, category } =
    req.body;
  let bookFind;
  try {
  ...
  bookFind.quantity = parseInt(quantity,10);
...
exports.placeOrder = async (req, res) => {
  ...
  try {
    ...
    if (bookFind.quantity < quantity) {
      return res.status(400).json({ msg: 'Not enough stock' });
    }
        // Giảm số lượng sách từ tồn kho
    bookFind.quantity -= quantity;
    await bookFind.save();
